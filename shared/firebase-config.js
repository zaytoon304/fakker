/*
  فكّر — إعداد Firebase (اختياري)

  المنصة تشتغل ١٠٠٪ بدون Firebase (التقدم يُحفظ على نفس الجهاز عبر localStorage).
  لو تبي التقدم يتزامن بين الأجهزة (مثلاً جوال الطالب + تابلت الفصل)، سوّي هذا:

  ١. روح لـ https://console.firebase.google.com وسوّي مشروع جديد اسمه "fakker"
...
function fakkerInitFirebase() {
  if (FIREBASE_CONFIG.apiKey === "YOUR_API_KEY") {
    console.log("فكّر: Firebase غير مُفعّل بعد — التقدم يُحفظ على هذا الجهاز فقط.");
    return null;
  }
  try {
    const app = firebase.apps && firebase.apps.length ? firebase.app() : firebase.initializeApp(FIREBASE_CONFIG);
    console.log("فكّر: Firebase متصل ✅");
    return firebase.database(app);
  } catch (err) {
    console.warn("فكّر: تعذّر الاتصال بـ Firebase، سيتم الاعتماد على الحفظ المحلي فقط.", err);
    return null;
  }
}
